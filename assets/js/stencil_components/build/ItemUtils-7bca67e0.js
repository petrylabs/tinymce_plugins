...
class ItemUtils {
    static generateConfigurationFromItemOptions(element) {
        ...
        components.forEach((item) => {
            configuration.add(new Item(item.hasAttribute('key') ? item.getAttribute('key') : '', item.hasAttribute('value') ? item.getAttribute('value') : '', item.hasAttribute('disabled') ? Boolean(item.getAttribute('disabled')) : false, item.hasAttribute('selected') ? Boolean(item.getAttribute('selected')) : false));
        });
