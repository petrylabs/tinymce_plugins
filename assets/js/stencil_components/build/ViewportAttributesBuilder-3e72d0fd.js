...
class ViewportConfigurationService {
    ...
    register(target, nodes, callback) {
        ViewportObserverManager.getInstance().observe(nodes[0].getRight(), callback.bind(target));
    }
    process(target, context, properties, entries) {
        ...
        properties.forEach(entry => {
            ...
            if (configuration.hasKey(propertyName)) {
                let parameters = configuration.getParametersByKey(propertyName).filter(parameter => parameter.inRange(Math.round(entries[0].contentRect.width), Math.round(entries[0].contentRect.height)));
