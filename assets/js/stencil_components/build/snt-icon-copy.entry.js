...
const IconCopyComponent = class {
    ...
    async registerViewport(target) {
        ViewportConfigurationService.getInstance().register(this, [Pair.of('snt-layout', target)], this.resize);
    }
    componentDidLoad() {
    ...
    resize(entries) {
        if (this.logoIsSet) {
            ViewportConfigurationService.getInstance().process(this, this.context, [Pair.of('logo', this.image)], entries);
        }
        else {
            ViewportConfigurationService.getInstance().process(this, this.context, [Pair.of('icon', this.image)], entries);
        }
    }
    render() {
        var content;
        ...
        return (h(Host, null, h("div", { class: "icon-copy-panel" }, content, h("h3", null, this.titleString), h("div", { class: WebUtils.generateClasses(["content"], this.styleClasses) }, h("slot", null))), h("snt-data", { type: "json", src: this.propertiesSrc, data: this.properties, ref: (el) => this.data = el })));
    }
