...
const embedChange = (tinymce.Env.ie && tinymce.Env.ie <= 8) ? 'onChange' : 'onInput';
...
const addEmbedHtml = function (win, editor) {
  return function (response) {
    ...
    const data = tinymce.util.Tools.extend(HtmlToData.htmlToData(Settings.getScripts(editor), html), { source1: response.url });
    win.fromJSON(data);
...
const populateMeta = function (win, meta) {
  tinymce.util.Tools.each(meta, function (value, key) {
...
const showDialog = function (editor) {
  ...
  const updateValueOnChange = function () {
    data = tinymce.util.Tools.extend({}, HtmlToData.htmlToData(Settings.getScripts(editor), this.value()));
