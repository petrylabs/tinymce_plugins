...
const dataToHtml = function (editor, dataIn) {
  const data = tinymce.util.Tools.extend({}, dataIn);

  if (!data.source1) {
    tinymce.util.Tools.extend(data, HtmlToData.htmlToData(Settings.getScripts(editor), data.embed));
    if (!data.source1) {
      return '';
    }
  }
  ...
  if (data.embed) {
    ...
    let mediaEmbedHtml;
    if (data.type === 'iframe') {
      mediaEmbedHtml = getIframeHtml(data);
    } else if (data.source1mime === 'application/x-shockwave-flash') {
      mediaEmbedHtml = getFlashHtml(data);
    } else if (data.source1mime.indexOf('audio') !== -1) {
      mediaEmbedHtml = getAudioHtml(data, audioTemplateCallback);
    } else if (data.type === 'script') {
      mediaEmbedHtml = getScriptHtml(data);
    } else {
      mediaEmbedHtml = getVideoHtml(data, videoTemplateCallback);
    }
    return '<span class="media-wrapper">' + mediaEmbedHtml + '</span>';
  }
};
